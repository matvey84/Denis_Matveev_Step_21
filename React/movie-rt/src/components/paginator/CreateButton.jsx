  ...
  props.getFetchWithButtonPage(e.target.id)//fetch request
	...
	return(
		<div className="btn-page-block">
			...
			<button onClick={(e)=>setNumBtn(numBtn-data.results.length)} id={'left-arrow'} className="btn-arrow left-arrow">
				<span className="btn-page-text">Back</span>
			</button>
			  	{buttonList}
			<button onClick={(e)=> setNumBtn(numBtn+data.results.length)} id={'right-arrow'} className="btn-arrow right-arrow">
