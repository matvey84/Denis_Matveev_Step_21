...
function Paginator() {
	...
	const totalResult = Number(data.total_results)
	const maxLimitPages = Math.ceil(totalResult/data.results.length)
  ...
  const parsedQueryString = useMemo(()=>queryString.parse(location.search),[location.search, queryString])
	...
	for(let i=1; i<=data.total_results; i++){
	arr.push(i)
	}
...
if((numIndex*10)>=maxLimitPages-data.results.length){
	hideRight.push('hide')
} 
