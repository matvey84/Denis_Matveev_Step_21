...
export const fetchMovie = (title='', type='multi', year='', page=1, ) => {

		//const apiKey = 'f5716cf9';
		//let url = `http://www.omdbapi.com/?apikey=${apiKey}&s=${title}&type=${type}&y=${year}&page=${page}`
const apiKey='435fe094379c7a46ee18b338ee38b3b2'
		let url =`https://api.themoviedb.org/3/search/${type}?api_key=${apiKey}&query=${title}&include_adult=false`
		console.log(url)
...
export function getSimilarResults (title=''){
	const apiKey='435fe094379c7a46ee18b338ee38b3b2'
		let url =`https://api.themoviedb.org/3/search/multi/?api_key=${apiKey}&query=${title}&include_adult=false`
	console.log(url)
	return async dispatch=>{
		try{
			const resp = await fetch(url);
...
export  function getFetchForDetailsInfo(obj){
	const {id,type}=obj
	// const apiKey = 'f5716cf9';
	// 	  const url = `http://www.omdbapi.com/?apikey=${apiKey}&i=${id}&plot=full`
	const apiKey='435fe094379c7a46ee18b338ee38b3b2'
	const url =`https://api.themoviedb.org/3/${type}/${id}?api_key=${apiKey}&append_to_response=videos`
			console.log(url)
				return async dispatch =>{		
			  try{
					const resp = await fetch(url);
...
//---------------------------------------
export const GET_CAST_INFO_ABOUT_SELECTED_MOVIE = 'GET_CAST_INFO_ABOUT_SELECTED_MOVIE'
export const getCastInfoAboutSelectedMovie = (obj)=>{
	const{type,id}=obj
	const apiKey='435fe094379c7a46ee18b338ee38b3b2'
	const url =`https://api.themoviedb.org/3/${type}/${id}/credits?api_key=${apiKey}&language=en-US`
			console.log(url)
				return async dispatch =>{		
			  try{
					const resp = await fetch(url);
					...
					setTimeout(()=>{
						resp.ok && dispatch({type:GET_CAST_INFO_ABOUT_SELECTED_MOVIE, payload:castData})
						castData.Response === 'False' && dispatch(showAlert(`${castData.Error} Введите полное название фильма!`))
						dispatch(hideLoading())
				},500)
				}catch(e){
					dispatch(showAlert('Возникли проблемы с загрузкой файла.'))
